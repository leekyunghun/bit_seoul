...
def autoencoder(filters_size, kernel_size):
    model = Sequential()
    model.add(Conv2D(filters = 784, kernel_size = kernel_size, padding = 'same', input_shape = (32, 32, 3), activation='relu'))
    ...
    model.add(Dropout(0.4))
    model.add(Conv2D(filters = filters_size, kernel_size = kernel_size, padding = 'same', activation='relu'))
    model.add(Conv2D(filters = 3, kernel_size = kernel_size, padding = 'same', activation='softmax'))
...
model.fit(x_train_noised, x_train_noised, epochs = 3, batch_size=100)
