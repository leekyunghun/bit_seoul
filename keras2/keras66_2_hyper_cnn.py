...
from tensorflow.keras.layers import Dense, Conv2D, MaxPooling2D, Flatten, Input, Dropout, Activation
...
def build_model(activation, drop = 0.5, optimizer = Adam, learning_rate = 0.001, node = 100):
    ...
    x = Conv2D(node, (2,2), name = 'conv2d_1')(inputs)
    x = Activation(activation)(x)
    x = Dropout(drop)(x)

    x = Conv2D(node, (2,2), name = 'conv2d_2')(x)
    x = Activation(activation)(x)
    x = Dropout(drop)(x)

    x = Conv2D(node, (2,2), name = 'conv2d_3')(x)
    x = Activation(activation)(x)
    x = Dropout(drop)(x)
...
# {'optimizer': 'rmsprop', 'node': 64, 'learning_rate': 0.01, 'epochs': 50, 'drop': 0.3, 'batch_size': 30, 'activation': 'relu'}
# 334/334 [==============================] - 1s 2ms/step - loss: 0.1625 - accuracy: 0.9888
# 최종 스코어 :  0.9887999892234802